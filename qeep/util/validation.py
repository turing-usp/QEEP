...
def _getOnePrediction(model: torch.nn, class_names: List[str], image_array: List[int], loader: transforms.Compose = None, quiet: bool = False) -> torch.Tensor:
    """
    Descrição
    --------
...
def getPredictions(model: torch.nn, class_names: List[str], image_arrays: List[int], quiet: bool = False) -> List[torch.Tensor]:
    """
    Descrição
    --------
    ...
    for image_array in image_arrays:
        outputs.append(_getOnePrediction(model, class_names = class_names, loader = loader, image_array = image_array, quiet = quiet))
    return outputs
