...
def classify_rois(model: torch.nn.Module, rois: np.array, classes: List[str]) -> List[Tuple[float, float, float, float]]:
	"""Classifica as ROIs
	# Entradas
		model: modelo
...
def filter_detections(original_img:Image, preds:List[tuple], locs:np.array, min_conf:float, visualize:int) -> Type[None]:
	"""Mostra as predições
	# Entradas
